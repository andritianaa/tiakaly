  ...
  return (
    <PopoverPrimitive.Trigger ref={forwardedRef as any} asChild>
      <div
        ...
        {...props}
        className={ny(
          "border-input ring-offset-background focus:ring-ring flex size-full min-h-10 h-fit items-center justify-between whitespace-nowrap rounded-md border bg-transparent px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-1 [&>span]:line-clamp-1",
    ...
    <PopoverPrimitive.Portal forceMount>
      ...
      >
        <Command
          ...
          filter={(value, search) => {
            if (!search) return 1;
            // Case insensitive search that checks if the value contains the search string
...
>(
  (
    ...
    const item = React.useMemo(() => {
      return value
        ? {
            ...
            name: name || (typeof children === "string" ? children : undefined),
          }
        : undefined;
    }, [value, label, children, name]);
    ...
    return (
      <CommandItem
        {...props}
        value={name || children?.toString() || label?.toString() || value}
        className={ny(
...
export type MultiSelectOption =
  | Pick<
      MultiSelectItemProps,
      "value" | "label" | "disabled" | "onSelect" | "onDeselect" | "name"
    >
