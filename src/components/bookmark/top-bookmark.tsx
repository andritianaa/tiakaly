...
import { toggleBookmarkTop } from '@/actions/bookmark-actions';
import { Button } from '@/components/ui/button';
import { useToast } from '@/hooks/use-toast';
import { useUser } from '@/hooks/use-user';
import { cn } from '@/lib/utils';

export type TopBookmarkProps = {
  topId: string;
  className?: string;
  /**
...
export const TopBookmark = ({
  ...
  useEffect(() => {
    const isBookmarked = user?.BookmarkTop?.includes(topId) ?? false;
  ...
  const handleBookmarkClick = async () => {
    if (!user) {
      toast({
        ...
        description: "Veuillez vous connecter pour enregistrer ce top",
        variant: "error",
        action: {
          label: "Se connecter",
    ...
    try {
      await toggleBookmarkTop(topId);
      setIsBookmarked((prev) => !prev);

      toast({
        title: isBookmarked ? "Retiré des favoris" : "Ajouté aux favoris",
        description: isBookmarked
          ? "Ce top a été retiré de vos favoris"
          : "Ce top a été ajouté à vos favoris",
      });
    } catch (error) {
      toast({
